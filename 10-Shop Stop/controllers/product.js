...
module.exports.editGet = (req, res) => {
    ...
    Product.findById(id).then(product => {
        ...
        if (
            ...
            res.redirect(`/?error=${encodeURIComponent("You can't do that!")}`);
            return;
        }
    });
};
    ...
    Product.findById(id)
        ...
        .then(product => {
            ...
            Category.findById(product.category).then(currentCategory => {
                ...
                index = req.user.createdProducts.indexOf(product._id);
                if (index >= 0) {
...
module.exports.buyPost = (req, res) => {
    ...
    Product.findById(productId).then(product => {
        if (product.buyer) {
            let error = `error=${encodeURIComponent(
                "Product was already bought!"
            )}`;
            res.redirect(`/?${error}`);
            return;
        }
